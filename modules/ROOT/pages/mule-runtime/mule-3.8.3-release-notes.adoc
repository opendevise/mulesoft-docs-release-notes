...
* Availability of the xref:api-manager::gatekeeper.adoc[gatekeeper feature] on clusters
+
...
The default policy polling time has been extended to reduce the load on API Manager and improve scalability. MuleSoft based this change on careful examination of actual customer experiences. A minimum 60-second delay now occurs between the time you apply a policy in the UI and the time the policy actually goes into effect. The same delay occurs when you disable or edit a policy. You can xref:api-manager::using-policies.adoc#configuring-the-policy-polling-time[configure the length of the delay].
