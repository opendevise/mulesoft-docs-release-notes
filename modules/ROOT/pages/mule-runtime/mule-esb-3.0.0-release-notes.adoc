...
*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/mule-deployment-model[Deployment Model]* - Mule now defines a simple application model that allow individual services or groups of services to be deployed as packaged applications into the Mule run-time.

*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/hot-deployment[Hot Deployment]* - Mule now supports multiple applications running within the same Mule instance with complete isolation from other applications in the container. Development time automatic hot deployment is supported with new changes being affected within milliseconds.
...
*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/error-handling[Exception Handling]* - Exception strategies were simplified to provide more consistent and predictable error-handling behavior.

*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/transformer-annotation[Automatic Transformation]* - The transformation engine has been enhanced to support Data bindings such as XML/JAXB and JSON/Jackson. Also Mime types are now supported when discovering transformers.

*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/cxf-module-reference[CXF Now a Module]* - Mule 3.0 architectural changes bring much better support for CXF, meaning it can be used just like another pipe/filter element.
...
*JBoss jBPM* - Mule has had integration with http://www.jboss.org/jbpm[JBoss jBPM] for a long time, but we have recently improved our support for Mule 3.0, including an upgrade to the latest major version of jBPM (4.4), much simplified configuration, and cleaner integration with Mule from your process definition, including custom process elements. For more information, refer to our https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/bpm-transport-reference[BPM documentation].
...
*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/xquery-transformer[XQuery Support]* - The XQuery Module gives users the ability to perform XQuery transformations on XML messages in Mule. This works in a very similar way to the XSLT Transformer shipped with Mule.

*https://docs.archive.mulesoft.com/mule-user-guide/v/3.2/schema-documentation[Schema Documentation]* -
