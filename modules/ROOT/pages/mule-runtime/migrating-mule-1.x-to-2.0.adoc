...
:page-aliases: migrating-mule-1.x-to-2.0.adoc
...
In 1.x, you started the Admin Agent by specifying a value for the `serverUrl` attribute in the `<mule-environment-properties>` element. In 2.x, you use the `<remote-dispatcher-agent>` element instead (see https://www.mulesoft.org/docs/site/current/apidocs/org/mule/module/client/remoting/RemoteDispatcherAgent.html[org.mule.module.client.remoting.RemoteDispatcherAgent]).
