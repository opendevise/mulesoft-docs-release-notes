...
|Software |*Version*
|*ESB Runtime* |3.7.0
|*Anypoint Studio* |xref:studio/anypoint-studio-june-2015-with-3.7.0-runtime-release-notes.adoc[June 2015]
|*MMC* |3.7.0
|*Anypoint DevKit* |xref:connector-devkit/anypoint-connector-devkit-3.7.0-release-notes.adoc[3.7.0]
...
* xref:3.7@mule-runtime::dataweave.adoc[DataWeave] (EE only) MuleSoft's new data transformation and query language
* xref:3.7@mule-runtime::improving-performance-with-the-kryo-serializer.adoc[Kryo serializer] (EE only)
* xref:3.7@mule-runtime::web-service-consumer.adoc[WS Consumer] support for WS-Security encryption and signatures.
* xref:3.7@mule-runtime::http-request-connector.adoc[HTTP request SNI support]
* xref:3.7@mule-runtime::authentication-in-http-requests.adoc[HTTP NTLM support]
* xref:3.7@mule-runtime::flow-processing-strategies.adoc[Non-blocking processing] strategy for HTTP proxy
* Lifecycle improvements
* xref:3.7@mule-runtime::dependency-injection.adoc[Dependency injection]
* xref:3.7@mule-runtime::unifying-the-mule-registry.adoc[Registry improvements]
* Connect to databases dynamically by using MEL expressions in the xref:3.7@mule-runtime::database-connector.adoc[database configuration]
* Java 8 support
...
Note 1: If you need to use URI parameters add an org.apache.cxf.interceptor.URIMappingInterceptor interceptor to the CXF service in your flow, see http://cxf.apache.org/cve-2012-5633.html[Apache CXF].
...
MULE-8583 also introduced hostname validation in the HTTP requester since the upgrade included the fix for https://github.com/AsyncHttpClient/async-http-client/issues/197[this] async-http-client security vulnerability. This means that seemingly valid certificates for HTTPS connections in older versions could now be rejected if they contain no Subject Alternative Name extension matching the host of the request.
...
|SEC-240 |Mule ESB 3.7.0 requires version of xref:api-manager::external-oauth-2.0-token-validation-policy.adoc[Anypoint Enterprise Security] to be 1.5.0 or greater
...
* xref:5@studio::index.adoc[Anypoint Studio]
