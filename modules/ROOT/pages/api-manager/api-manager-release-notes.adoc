...
* A tight integration of Anypoint Platform components extends the xref:api-manager::environments-concept.adoc[use of environments] to API management.
* API permissions are now environment-based.
* You can now xref:api-manager::create-instance-task.adoc[create multiple instances] of an API.
* APIs in Exchange can be xref:api-manager::manage-exchange-api-task.adoc[managed from API Manager].
...
** This release of Mule 4 xref:api-manager::policy-scope-size-concept.adoc[changes the packaging, size, and scope] of a policy.
** All policies are non-blocking.
** All policies except CORS, which is executed first, can be ordered.
* xref:api-manager::develop-custom-policies-reference.adoc[Custom policy development] has changed.
...
* API Manager xref:api-manager::latest-overview-concept.adoc[does not support auto-generation and deployment of proxies].
...
This feature provides the flexibility to apply policies to specific resources and/or methods of an API instead to the entire API. This feature is enabled only for RAML-based APIs or proxies. For more information, see xref:api-manager::resource-level-policies-about.adoc["About Resource Level Policies"] and xref:api-manager::tutorial-manage-an-api.adoc["To Apply Policies and SLA Tiers."]

Custom policies require changes to enable Resource Level granularity. For more information, see xref:1.x@api-manager::add-rlp-support-task.adoc["To Add Resource Level Policy Support to a Custom Policy."]
...
+
You can now specify multiple throughput limits for an SLA tier using xref:api-manager::rate-limiting-and-throttling-sla-based-policies.adoc#creating-an-sla-tier-or-layered-slas[different time periods and units].
...
* The capability to xref:api-manager::using-api-alerts.adoc[set alerts] for policy violations, response time, count, and codes.
* An advanced configuration to customize the response timeout of the xref:1.x@api-manager::setting-up-an-api-proxy.adoc[auto-generated proxy].
+
To customize the response timeout of the deployed proxy, use the advanced options as described in step 5-h of section "xref:1.x@api-manager::setting-up-an-api-proxy.adoc#setting-up-a-proxy[Setting up a Proxy]".
...
API Manager 1.13.0 includes the capability to xref:api-manager::using-policies.adoc#disabling-and-enabling-policies[enable and disable policies].

=== Resolved Issues and Improvements

API Manager 1.13.0 fixes API Tooling bugs to support new API tooling. API Manager has been improved to support dependencies between policies that you configure using the xref:1.x@api-manager::custom-policy-reference.adoc#policy-definition-yaml-file[required characteristics] parameter.
...
API Manager 1.10.0 includes the capability to sort the list of API Portals and the list of applications on the Developer Portal, as described in the xref:1.x@api-manager::engaging-users-of-your-api.adoc#developer-portal[Developer portal documentation].
...
//+
//You can search, browse, and sort the apps listed on //Applications tab on the xref:1.x@api-manager::tutorial-set-up-and-deploy-an-api-proxy.adoc#navigate-to-the-api-version-details-page[API version details] page.
...
+
When you apply xref:api-manager::client-id-based-policies.adoc[client ID-based policies], the credentials are expected in the form of expressions, the default being query parameters named client_id and client_secret. The default configuration of the policy requires the xref:api-manager::client-id-based-policies.adoc#required-fields-in-api-calls[Client Id and Client Secret expressions]. In Mule Runtime 3.8.0, another option is available. You can also select HTTP Basic Authorization Header to use Basic Authentication as the origin of the credentials.
+
...
* xref:api-manager::defining-sla-tiers.adoc[Layered SLAs]﻿ that can impose multiple limiting policies on the API.
...
* xref:1.x@api-manager::engaging-users-of-your-api.adoc#adding-terms-and-conditions[Adding Terms and Conditions] ﻿of an API Portal can now be defined in two different ways.
* Files and Attachments can be imported/exported to an xref:1.x@api-manager::engaging-users-of-your-api.adoc[API Portal]﻿.
...
* xref:1.x@api-manager::configuring-an-api-gateway.adoc[New API Gateway distribution] with enhanced API Autodiscovery, including Autodiscovery for APIkit projects, and improved HTTP transport performance using an NIO-based transport.
* xref:1.x@api-manager::setting-up-an-api-proxy.adoc[Automatic proxy generation] for API endpoints defined by HTTP, WSDL, and RAML.
...
* xref:1.x@api-manager::browsing-and-accessing-apis.adoc[Streamlined application registration and management] for application developers.
* xref:api-manager::viewing-api-analytics.adoc[New, robust API Analytics] with customizable charts and dashboards and export capabilities.
* xref:access-management::external-identity.adoc[External identity management] support with PingFederate.
* xref:api-manager::using-policies.adoc[Three new governance policies]: PingFederate Access Token Enforcement, JSON Threat Protection, XML Threat Protection.
...
* The administrative view of your API version (called the xref:1.x@api-manager::tutorial-set-up-and-deploy-an-api-proxy.adoc#navigate-to-the-api-version-details-page[API version details page]) is now accessible only to API Version Owners or Organization Administrators. The Developer Portal, containing the API Portals that you create and share, now acts as the developer-facing view of your API.
* xref:api-manager::using-policies.adoc[Policy application] has been streamlined to a single step for each policy. Contract enforcement and related policies have been replaced with client ID and secret enforcement. SLA-based policies now incorporate client ID and secret enforcement automatically.
* xref:1.x@api-manager::browsing-and-accessing-apis.adoc[Application management] flows have changed. You can now set SLA tiers for automatic approval to reduce your management overhead. Manual approval is also available.
* xref:api-manager::viewing-api-analytics.adoc[Analytics] are now available only to Organization Administrators.
