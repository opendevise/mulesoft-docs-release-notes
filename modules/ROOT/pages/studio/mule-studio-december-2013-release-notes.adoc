...
These release notes apply to Mule Studio (December 2013) and the CloudHub Mule Runtime (December 2013). Note that the new features and functionality described here are *_only available for deployment in CloudHub_* using the CloudHub Mule Runtime (December 2013). If you intend to deploy your application to an on-premises production server, use a Mule 3.4.1 runtime when you begin a new project in Mule Studio.
...
Access the xref:cloudhub/cloudhub-release-notes.adoc[Release Notes for the December 2013 release of CloudHub].  +
Access the xref:1.x@api-manager::api-gateway-runtime-archive.adoc[API Gateway Runtime].
...
* **xref:3.7@mule-runtime::batch-processing.adoc[Batch processing]:** New in Mule, use batch processing functionality to process data sets – particularly large or streaming – in batches to maximize data integration efficiency.
* **xref:3.7@mule-runtime::mule-expression-language-mel.adoc[Auto-completion for MEL]:** All configuration parameters in Studio now support auto-completion of MEL expressions.
* *xref:3.7@mule-runtime::poll-schedulers.adoc[Poll scheduler]:* Supports adjusting the default polling interval and other more complex polling logic, including Cron expressions.
* *xref:3.7@connector-devkit::adding-datasense.adoc[DataSense Usability Improvements]:* Added time out functionality so as to facilitate quicker response to 'cancel' operation.
...
* Having joined the API Platform family of products, the Anypoint Service Registry has a new name: xref:api-manager::index.adoc[API Manager]. Further, it has has some new functionality: **xref:3.8@mule-runtime::setting-up-ldap-provider-for-spring-security.adoc[LDAP Security Provider Policy]**, xref:api-manager::using-policies.adoc[Consumer Key enforcement], and *Endpoint aliasing*.
* *New and Improved CloudHub features:* New Enterprise xref:runtime-manager::virtual-private-cloud.adoc[Virtual Private Cloud] and support for xref:runtime-manager::managing-schedules.adoc[poll scheduling] and xref:3.7@mule-runtime::batch-processing.adoc[batch processing].
...
|===
|HABITAT-1194 a|
Services, proxies, or APIs deployed to the December 2013 release on CloudHub and manually registered in API Manager using the CloudHub endpoint URL do not support runtime policies. For more details, see the xref:api-manager/api-manager-release-notes.adoc[API Manager release notes].
...
|DEVKIT-196 |INCLUDE_ERROR code snippets in generated docs.  `javadoc:javadoc` is passed as an argument
...
|https://www.mulesoft.org/jira/browse/STUDIO-3825[STUDIO-3825] |handleOutputResultSets in `jdbc-ee:connector` not recognized by Studio
...
|https://www.mulesoft.org/jira/browse/STUDIO-3926[STUDIO-3926] |After adding a Datamapper mp to a flow, the palette Filter no longer works.
...
|https://www.mulesoft.org/jira/browse/STUDIO-3983[STUDIO-3983] |Batch: `batch:input` is not present in autocompletion
...
|https://www.mulesoft.org/jira/browse/STUDIO-4080[STUDIO-4080] |Batch :: 'Name must be unique' validation message when executing another batch with `batch:execute`
...
|https://www.mulesoft.org/jira/browse/STUDIO-4139[STUDIO-4139] |Templates and Keywords shouldn't be suggested when an identifier is resolved
...
|https://www.mulesoft.org/jira/browse/STUDIO-3613[STUDIO-3613] |Editor for `batch:execute`
...
At this time, not all of the third-party extensions you may have been using with previous versions of Mule ESB have been upgraded to work with Mule Studio (December 2013). https://www.mulesoft.com/support-and-services/mule-esb-support-license-subscription[Contact MuleSoft Support] if you have a question about a specific module.

== Support Resources

* Refer to MuleSoft’s online documentation at {site-url}[MuleSoft Docs] for instructions on how to use the new features and improved functionality in Mule Studio (December 2013) with CloudHub Mule Runtime (December 2013).
