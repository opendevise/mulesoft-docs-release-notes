...
:page-aliases: anypoint-studio-march-2015-with-3.6.1-runtime-release-notes.adoc
...
+
*Note*: If you are installing Anypoint Studio on a new computer or a fresh installation, you must also download and install http://www.oracle.com/technetwork/java/javase/downloads/java-archive-downloads-javase6-419409.html[JDK 1.6], which provides important libraries for later JDK versions. Install JDK 1.6 first and then install JDK 1.7.

[TIP]
To replace the deprecated HTTP connector for the new HTTP connector, refer to xref:3.7@mule-runtime::migrating-to-the-new-http-connector.adoc[Migrating to the New HTTP Connector].
...
* For further details on Anypoint Studio with 3.6.1 Runtime, see the xref:mule-runtime/mule-esb-3.6.1-release-notes.adoc[Mule ESB 3.6.1 Release Notes]
* Refer to MuleSoft’s {site-url}[documentation] - for instructions on how to use the new features and improved functionality in Anypoint Studio with 3.6.1 Runtime.
* Access MuleSoft’s http://forums.mulesoft.com[MuleSoft Forums] - to pose questions and get help from Mule’s broad community of users.
