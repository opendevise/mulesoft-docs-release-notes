...
* *xref:3.7@mule-runtime::database-connector.adoc[New Database Connector]*: This new connector allows you to connect with almost any JDBC relational database, consistently using the same interface for every case. You can run diverse SQL operations on your database, including Select, Insert, Update, Delete, and even Stored Procedures.
* *xref:3.7@mule-runtime::web-service-consumer.adoc[New Web Service Consumer]*: This new connector allows you to consume a web service from within your Mule application using information automatically extracted from the service's WSDL using DataSense.
* *Palette Simplification*: In the Studio palette, Endpoints and Cloud Connectors categories have been merged in one category called xref:3.7@mule-runtime::anypoint-connectors.adoc[Connectors].
* *xref:3.7@mule-runtime::batch-streaming-and-job-execution.adoc[Batch Processing Improvements]*: Batch Processing now includes the ability to batch process non-serializable messages, and to stream batch commits. Further, you can now order the execution of batch job instances.
* *xref:3.7@mule-runtime::shared-resources.adoc[Shared Resources]*: Need to share connectors and transaction managers between applications? Shared Resources gives you the ability to share configurations between applications and development teams, including the new Database Connector and the new Bitronix transaction manager.
* *xref:3.7@mule-runtime::scatter-gather.adoc[Scatter-Gather]*:A routing message processor, Scatter-Gather, sends a request message to multiple targets concurrently. It collects the responses from all routes and aggregates them into a single message, but still allows you to implement sequential routing if desired.
* *xref:5@studio::using-the-datasense-explorer.adoc[DataSense Explorer]*: This feature surfaces information about the Mule message – payload, variables, and properties – as it passes through your flow, relative to the building block you currently have selected on the canvas.
* *xref:3.7@mule-runtime::invoke-component-reference.adoc[Invoke Message Processor]*: The Invoke message processor is newly exposed in the Studio interface.
* *xref:3.7@mule-runtime::request-reply-scope.adoc[Request-Reply Scope]*: The Request-Reply scope is newly exposed in the Studio interface.
* *xref:3.7@mule-runtime::fips-140-2-compliance-support.adoc[Support for FIPS 140-2 Compliance]*: Mule now can be configured to run in a FIPS 140-2 certified environment.
* *xref:3.7@mule-runtime::until-successful-scope.adoc[Synchronous Until-Successful]*: New with 3.5.0, you can configure your until-successful scope to process messages synchronously, relative to your main flow.
* *https://docs.archive.mulesoft.com/mule-management-console/v/3.7/analyzing-flow-processing-and-payloads[Enhanced Flow Analyzer in MMC]*: We have enhanced the Flow Analyzer functionality by adding the ability to stop analysis automatically after a period of time. Further, privileged users can stop an ongoing analysis session.
* *Updates to MEL syntax*: Mule supports xref:3.7@mule-runtime::mule-expression-language-reference.adoc[dot syntax] in MEL expressions. In Anypoint Studio, autocomplete supports this dot syntax for all object fields. However, you must use the bracket syntax for map access in cases where the keys are not strings or you need to evaluate an expression to obtain the actual key to use.
...
* *Upgraded HA*: We have upgraded the Hazelcast engine underlying Mule's High Availability clustering and added the ability to run HA clusters in https://docs.archive.mulesoft.com/mule-management-console/v/3.7/managing-mule-high-availability-ha-clusters[performant mode] to prioritize performance over reliability.
...
* *Anypoint Connector DevKit in Studio*: To build your own Anypoint Connector, you can use the xref:3.7@connector-devkit::index.adoc[Anypoint Connector DevKit]. In the latest release of Anypoint Studio, you can add a Beta module that enables you to access DevKit functionality directly in Studio. Includes: +
...
* *xref:1.x@api-manager::api-gateway-runtime-archive.adoc[API Gateway Runtime]:* Included is an agent that includes several fixes and improves performance for contract-based policies.
...
|===
|Issue |Description
|n/a |xref:3.7@mule-runtime::batch-processing.adoc[Batch processing] does not support the use of https://docs.archive.mulesoft.com/mule-management-console/v/3.7/migration-scripts-for-business-events[Business Events].
|n/a |xref:runtime-manager::insight.adoc[Insight] does not support visibility into batch processing.
...
|===
|Issue |Description
|n/a |https://docs.archive.mulesoft.com/mule-management-console/v/3.7/[Mule Management Console (MMC)] does not support visibility into batch processing.
...
|MULE-6916 |Update Saxon version to 9.1.0.8. See <<junittestfailnote,JUnit tests fail note>>.
...
The 3.5.0 version of MMC requires migration steps that vary according to the version and setup of the MMC you are migrating from. Please either perform a clean install or follow the steps targeted to your current version in the instructions for https://docs.archive.mulesoft.com/mule-management-console/v/3.7/upgrading-the-management-console[Upgrading the Management Console].
...
With the above-listed versions of OAuth-enabled connectors, the OAuth support in Mule changed so that, by default, all tokens are stored under a key that matches the *connector’s config name*. https://blogs.mulesoft.com/dev/mule-dev/mule-oauth2-support-even-easier-still/[Read blog example.]
...
See also <<junittestfailnote,JUnit Test Fail Note>>.
...
* Refer to MuleSoft’s {site-url}[Documentation]
* xref:3.5@mule-runtime::index.adoc[3.5 Mule Runtime]
