...
* AGW-412: On xref:1.x@api-manager::api-auto-discovery.adoc[API Autodiscovery], add a flag to allow the possibility to disable API creation.
* AGW-421: Add a unique key to the Analytics event data
* AGW-434: xref:api-manager::defining-sla-tiers.adoc[Layered Policies]
* AGW-496: Ability to configure CORS on an xref:api-manager::oauth2-provider-configuration.adoc#applying-cors[External OAuth 2.0 Provider Application]
...
* xref:api-gateway/api-gateway-2.0.2-release-notes.adoc[API Gateway Runtime 2.0.2 Release Notes]