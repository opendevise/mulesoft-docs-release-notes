...
* External AES OAuth2 validation policy. See  xref:api-manager::external-oauth-2.0-token-validation-policy.adoc[External OAuth 2.0 Token Validation Policy].
* New HTTP Listener Connector. See  xref:3.7@mule-runtime::http-listener-connector.adoc[HTTP Listener Connector].
...
* Non-blocking support. See xref:3.7@mule-runtime::flow-processing-strategies.adoc#FlowProcessingStrategies-Non-BlockingProcessingStrategy[Non-Blocking Processing].
* Java 8 support.
...
* With the HTTP listener support, the old endpoint based version of the HTTP connector is no longer supported in 2.0. Use the xref:mule-runtime/migration-guide-to-api-gateway-2.0.0-or-later.adoc[migration tool] for existing proxies.
* Use register-as when defining autodiscovery for endpoints removed from the schema.

== Migration Guide

. Modify and migrate all deployed applications and proxies to use the new xref:3.7@mule-runtime::http-listener-connector.adoc[HTTP listener] used in the 2.0 runtime. For automatically generated (but not modified) proxies of versions 1.3.X or older, there is a xref:mule-runtime/migration-guide-to-api-gateway-2.0.0-or-later.adoc[migration tool.] However, custom apps or post-generation modified proxies will need to be manually modified and re-deployed.
. Custom policies continue to work. However as-is if policy violations occur, these are not reported to analytics as such events need to include their ID. To ensure that violations are reporting, the policy needs to be modified to include an ID. See xref:1.x@api-manager::applying-custom-policies.adoc[Applying Custom Policies] for examples of both runtimes.
