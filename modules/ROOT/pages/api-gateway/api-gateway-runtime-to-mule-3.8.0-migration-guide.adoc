...
As part of the xref:api-manager::gatekeeper.adoc[Gatekeeper security improvement], on startup, managed API traffic is disabled until the online policies are successfully applied. Until traffic is enabled, the API receives a 503 status code in response to requests.
...
API Manager continues to support generating a proxy for referencing the domain, as described in xref:1.x@api-manager::api-gateway-domain.adoc[API Gateway domain] documentation.
