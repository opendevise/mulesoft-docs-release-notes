...
After the upgrade, all APIs are listed in an environment called the Unclassified environment. From here, you can classify APIs into your appropriate environments. For the environments you classify APIs into, there is a new access control which is explained in https://docs.mulesoft.com/api-manager/v/2.x/environment-permission-task[To Grant Permission to Access Environments]. Existing API Manager roles and permissions continue to work in the Unclassified environment, and the APIs section in Access Management is applicable to unclassified APIs. 
